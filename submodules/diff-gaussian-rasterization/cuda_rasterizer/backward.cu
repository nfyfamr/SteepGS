...
__device__ void computePartialSplittingMatrix(int idx, const float3& m, const float* proj, const glm::vec3& dL_dmean, const float4& dL_dmean2D, glm::mat3* splitting_mats)
{
	...
	float mul3 = (proj[3] * m.x + proj[7] * m.y + proj[11] * m.z + proj[15]);

	glm::mat3x2 dmean2D_mean3D = glm::mat3x2(
		proj[0] * m_w - proj[3] * mul1, proj[1] * m_w - proj[3] * mul2,
		proj[4] * m_w - proj[7] * mul1, proj[5] * m_w - proj[7] * mul2,
		proj[8] * m_w - proj[11] * mul1, proj[9] * m_w - proj[11] * mul2);

	glm::vec3 px = glm::vec3(proj[0], proj[4], proj[8]);
	glm::vec3 py = glm::vec3(proj[1], proj[5], proj[9]);
	glm::vec3 pw = glm::vec3(proj[3], proj[7], proj[11]);

	glm::mat3 H_pix_mean3D = (-glm::outerProduct(px, pw) - glm::outerProduct(pw, px) + 2.f * glm::outerProduct(pw, pw) * mul1 * mul3) * m_w * m_w;
	glm::mat3 H_piy_mean3D = (-glm::outerProduct(py, pw) - glm::outerProduct(pw, py) + 2.f * glm::outerProduct(pw, pw) * mul2 * mul3) * m_w * m_w;

	glm::vec2 cov_x_minus_pi = glm::vec2(splitting_mats[idx][0][0], splitting_mats[idx][0][1]);

	glm::mat2x2 G_cov_dL_dG = glm::mat2x2(
		splitting_mats[idx][1][0], splitting_mats[idx][1][0],
		splitting_mats[idx][1][1], splitting_mats[idx][1][2]);

	glm::mat3x3 S = glm::transpose(dmean2D_mean3D) * glm::outerProduct(cov_x_minus_pi, dL_dmean);
	S -= glm::transpose(dmean2D_mean3D) * G_cov_dL_dG * dmean2D_mean3D;
	S += dL_dmean2D.x * H_pix_mean3D + dL_dmean2D.y * H_pix_mean3D;

	splitting_mats[idx] = S;
}
...
__global__ void preprocessCUDA(
	...
	glm::vec3* dL_dscale,
	glm::vec4* dL_drot,
	glm::mat3* splitting_mats,
	const CudaRasterizer::SplittingEstimator S_estimator)
{
	auto idx = cg::this_grid().thread_rank();
	if (idx >= P || !(radii[idx] > 0))
		return;
	...
	if (splitting_mats)
	{
		if (S_estimator == CudaRasterizer::InverseCov)
		{
			...
			splitting_mats[idx][1][1] = (proj[4] * m_w - proj[7] * mul1) * splitting_mats[idx][0][1] + (proj[5] * m_w - proj[7] * mul2) * splitting_mats[idx][0][2];
			splitting_mats[idx][1][2] = (proj[8] * m_w - proj[11] * mul1) * splitting_mats[idx][0][1] + (proj[9] * m_w - proj[11] * mul2) * splitting_mats[idx][0][2];
		}
		else if (S_estimator == CudaRasterizer::Partial)
		{
			computePartialSplittingMatrix(idx, m, proj, dL_dmean, dL_dmean2D[idx], splitting_mats);
		}
		else if (S_estimator == CudaRasterizer::Approx)
		{
			glm::mat2x3 J = glm::mat2x3(
				proj[0] * m_w - proj[3] * mul1, proj[4] * m_w - proj[7] * mul1, proj[8] * m_w - proj[11] * mul1, // first column
				proj[1] * m_w - proj[3] * mul2, proj[5] * m_w - proj[7] * mul2, proj[9] * m_w - proj[11] * mul2  // second column
			);

			glm::mat2x2 splitting_mat_2D = glm::mat2x2(
				splitting_mats[idx][0][0], splitting_mats[idx][0][1],
				splitting_mats[idx][0][1], splitting_mats[idx][0][0]
			);

			splitting_mats[idx] = J * splitting_mat_2D * glm::transpose(J);
...
__global__ void PerGaussianRenderCUDA(
	...
	glm::mat3* __restrict__ splitting_mat,
	const CudaRasterizer::SplittingEstimator S_estimator)
{
	auto block = cg::this_thread_block();
	auto warp = cg::tiled_partition<32>(block);
	uint32_t global_bucket_idx = block.group_index().x * warp.meta_group_size() + warp.meta_group_rank();
	...
	int bucket_idx_in_tile = global_bucket_idx - bucket_base;
	int splat_idx_in_tile = bucket_idx_in_tile * 32 + warp.thread_rank();
	...
	float dL_dcolors_accum[C] = { 0.0f };
	float splitting_accum[9] = { 0.0f };
	...
	for (int i = 0; i < BLOCK_SIZE + 31; ++i)
	{
		...
		if (valid_splat && valid_pixel && 0 <= idx && idx < BLOCK_SIZE)
		{
			...
			for (int ch = 0; ch < C; ++ch)
			{
				...
				dL_dcolors_accum[ch] += weight * dL_dchannel;
			...
			dL_dopacity_accum += G * dL_dalpha;

			if (S_estimator == CudaRasterizer::InverseCov)
			{
				splitting_accum[0] += dL_dG;
				splitting_accum[1] += dG_ddelx * ddelx_dx;
				splitting_accum[2] += dG_ddely * ddely_dy;
			}
			else if (S_estimator == CudaRasterizer::Partial)
			{
				splitting_accum[0] += -d.x * con_o.x - d.y * con_o.y;
				splitting_accum[1] += -d.y * con_o.z - d.x * con_o.y;
				splitting_accum[3] += dL_dG * con_o.x * G;
				splitting_accum[4] += dL_dG * con_o.y * G;
				splitting_accum[5] += dL_dG * con_o.z * G;
			}
			else if (S_estimator == CudaRasterizer::Approx)
			{
				const float dQ_ddelx = d.x * con_o.x + d.y * con_o.y;
				const float dQ_ddely = d.y * con_o.z + d.x * con_o.y;
				splitting_accum[0] += dL_dG * G * (dQ_ddelx * ddelx_dx * dQ_ddelx * ddelx_dx + con_o.x);
				splitting_accum[1] += dL_dG * G * (dQ_ddelx * ddelx_dx * dQ_ddely * ddely_dy + con_o.y);
				splitting_accum[4] += dL_dG * G * (dQ_ddely * ddely_dy * dQ_ddely * ddely_dy + con_o.z);
			}
		}
	}
	...
	if (valid_splat)
	{
		...
		atomicAdd(&dL_dopacity[gaussian_idx], dL_dopacity_accum);
		for (int ch = 0; ch < C; ++ch)
			atomicAdd(&dL_dcolors[gaussian_idx * C + ch], dL_dcolors_accum[ch]);
		for (int row = 0; row < 3; ++row)
			for (int col = 0; col < 3; ++col)
				atomicAdd(&splitting_mat[gaussian_idx][row][col], splitting_accum[col * 3 + row]);
	}
}
