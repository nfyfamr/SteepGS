	...
	const bool debug)
{
  ...
  torch::Tensor splitting_mats = torch::zeros({P, 3, 3}, means3D.options());
  ...
  if(P != 0)
  {
	  ...
	  splitting_mats.contiguous().data<float>(),
	  (CudaRasterizer::SplittingEstimator) S_estimator,
	  debug);
  }

  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, splitting_mats);
}
